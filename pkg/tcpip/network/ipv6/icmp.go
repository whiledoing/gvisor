...
func getLinkAddrOption(it header.NDPOptionIterator, getAddr func(header.NDPOption) tcpip.LinkAddress) (tcpip.LinkAddress, bool) {
	...
	for {
		...
		if err != nil {
			// This option is not valid as per the wire format, silently disregard.
			return "", false
		}
		if done {
			break
		...
		if addr := getAddr(opt); len(addr) != 0 {
			// No RFCs define what to do when an NDP message has multiple Source or
			// Target Link-Layer Address options. Since no interface can have
			// multiple link-layer addresses, we consider such messages invalid.
			if len(linkAddr) != 0 {
				return "", false
			}
			linkAddr = addr
...
func getSourceLinkAddr(it header.NDPOptionIterator) (tcpip.LinkAddress, bool) {
	return getLinkAddrOption(it, func(opt header.NDPOption) tcpip.LinkAddress {
		src, ok := opt.(header.NDPSourceLinkLayerAddressOption)
		if ok {
...
func getTargetLinkAddr(it header.NDPOptionIterator) (tcpip.LinkAddress, bool) {
	return getLinkAddrOption(it, func(opt header.NDPOption) tcpip.LinkAddress {
		dst, ok := opt.(header.NDPTargetLinkLayerAddressOption)
		if ok {
