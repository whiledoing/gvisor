...
func (n *NIC) addAddressLocked(protocolAddress tcpip.ProtocolAddress, peb PrimaryEndpointBehavior, kind networkEndpointKind, configType networkEndpointConfigType, deprecated bool) (*referencedNetworkEndpoint, *tcpip.Error) {
	...
	if n.neigh != nil {
		...
		// Explicitly set nud as a nil interface value so the NetworkEndpoints
		// don't confuse a nil value for a non-nil interface value. An interface
		// value is nil only if the value (V) and the type (T) are both unset
		// (T=nil, V is not set).
		//
